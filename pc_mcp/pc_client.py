...
def main(argv: list[str] | None = None) -> None:
    ...
    # --url/--timeout are also accepted after the subcommand (e.g. `pc.py call x --timeout 300`).
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--url", default=argparse.SUPPRESS, help=argparse.SUPPRESS)
    common.add_argument("--timeout", type=float, default=argparse.SUPPRESS, help=argparse.SUPPRESS)

    def add(name: str, help: str) -> argparse.ArgumentParser:
        return sub.add_parser(name, help=help, parents=[common])

    c = add("connect", help="save the connection URL and show system info")
    c.add_argument("connect_url")
    add("info", help="system overview")
    add("tools", help="list tools and parameters")
    c = add("call", help="call any tool")
    c.add_argument("tool")
    c.add_argument("args", nargs="*", help="one JSON object, or key=value pairs")
    c = add("run", help="run a shell command on the PC")
